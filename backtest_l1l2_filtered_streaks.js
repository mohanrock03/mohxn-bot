/**
 * ╔═══════════════════════════════════════════════════════════════════════╗
 * ║  BACKTEST: CONSECUTIVE L1+L2 FAILURES (FILTERED)                      ║
 * ║  6-Year Real-World Mix Data (2190 Days)                             ║
 * ║                                                                       ║
 * ║  Logic:                                                               ║
 * ║  1. If L1 Wins: IGNORE the session (don't break or count streak).      ║
 * ║  2. If L1 Loses and L2 Wins: BREAK streak (reset to 0).               ║
 * ║  3. If L1 Loses and L2 Loses: COUNT streak (+1).                      ║
 * ╚═══════════════════════════════════════════════════════════════════════╝
 */
...
function runFilteredStreakTest(data) {
    const WINDOW = 20;
    const THRESHOLD = 40;
    const MART_LEVELS = 5;

    let martLevel = 1;
    let currentFilteredStreak = 0;
    let maxFilteredStreak = 0;
    let streakDist = {};
    ...
    for (let i = WINDOW; i < data.length; i++) {
        ...
        if (isWin) {
            if (martLevel === 1) {
                l1Wins++;
                // L1 WIN: "not considered" -> skip everything, no streak change
            } else if (martLevel === 2) {
                l2Wins++;
                // L2 WIN: Streak BREAKS (L1 failed, but L2 recovered)
                if (currentFilteredStreak > 0) {
                    streakDist[currentFilteredStreak] = (streakDist[currentFilteredStreak] || 0) + 1;
                }
                currentFilteredStreak = 0;
            } else {
                // Win at L3, L4, or L5 -> streak already incremented at L2 failure
            }
            martLevel = 1;
        } else {
            // LOSS
            if (martLevel === 2) {
                // L1 failed AND L2 failed -> COUNT STREAK
                l1l2Failures++;
                currentFilteredStreak++;
                if (currentFilteredStreak > maxFilteredStreak) maxFilteredStreak = currentFilteredStreak;
            }
    ...
    return { maxFilteredStreak, streakDist, totalSessions, l1Wins, l2Wins, l1l2Failures };
}
...
console.log("╔═══════════════════════════════════════════════════════════════════════════╗");
console.log("║  BACKTEST: CONSECUTIVE L1+L2 FAILURES (FILTERED) — 6 YEARS             ║");
console.log("║  Logic: L1 Wins are IGNORED. Streak breaks only on L2 Win.             ║");
console.log("╚═══════════════════════════════════════════════════════════════════════════╝\n");

for (let [tf, rpd] of Object.entries(TIMEFRAMES)) {
    let totalRounds = rpd * DAYS;
    let data = genRealWorldMix(totalRounds);
    let result = runFilteredStreakTest(data);

    console.log(`┌─── TIMEFRAME: ${tf.toUpperCase()} ──────────────────────────────────────────┐`);
    console.log(`│  Total Sessions:     ${result.totalSessions.toLocaleString().padEnd(10)} (L1 Wins: ${result.l1Wins.toLocaleString()})    │`);
    console.log(`│  ★ MAX CONSECUTIVE L1+L2 FAILURES: ${String(result.maxFilteredStreak).padEnd(10)}            │`);
    console.log(`│                                                                 │`);
    console.log(`│  Distribution of Streaks (Sessions that failed L1 & L2):        │`);
    
    let sortedStreaks = Object.entries(result.streakDist).sort((a, b) => parseInt(a[0]) - parseInt(b[0]));
    for (let [len, count] of sortedStreaks) {
        if (len > 0) {
            console.log(`│    ${len}× in a row: ${String(count).padEnd(6)} times                            │`);
        }
    }
    console.log(`└─────────────────────────────────────────────────────────────────┘\n`);
}
